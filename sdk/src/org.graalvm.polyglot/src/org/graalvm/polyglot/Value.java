...
public final class Value {
     ...
     * Removes a member.
     *
     * @return <code>true</code> when the member was successfully removed, <code>false</code> otherwise.
     * @since 1.0
     */
    ...
    /**
     * Returns <code>true</code> whether this value represents a {@link Proxy}. The proxy instance can
     * be unboxed using {@link #asProxyObject()}.
     *
     * @since 1.0
     */
     ...
     * {@link ClassCastException} is thrown. Primitive class literals throw a
     ...
     * boolean}. Primitive {@link Boolean boolean.class} literal is also supported. The primitive class
     * literal throws a {@link NullPointerException} if the value represents {@link #isNull() null}.
     ...
     * component type is used, then the return types of the the list are subject to Object target type
     ...
     * the list are recursively subject to Object target type mapping rules.
     ...
     * executed}. The coercion to the parameter types of functional interface method is converted using
     * the semantics of {@link #as(Class)}. If a standard functional interface like {@link Function} are
     * used, is recommended to use {@link #as(TypeLiteral) type literals} to specify the expected
     * generic method parameter and return type.
     * <li>Any interface if the value {@link #hasMembers()}. Each method or field name maps to one
     * {@link #getMember(String) member} of the value. Whenever a method of the interface is executed a
     * member with the method or field name must exist otherwise a {@link UnsupportedOperationException}
     * is thrown when the method is executed. If one of the parameters cannot be mapped to the target
     * type a {@link ClassCastException} or a {@link NullPointerException} is thrown.
     * </ul>
     ...
     * maps it to a {@link Map}. Using the {@link Context#asValue(Object)} it is possible to recreate
     * the {@link Value polyglot value} from the Java map. The JavaScript object identity is preserved
     * in the process.
     *
     * <pre>
     * Context context = Context.create();
