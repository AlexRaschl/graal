...
public final class StackValue {
     ...
     * returned pointer is aligned on a word boundary. If the requested size is 0, the method returns
     * {@code null}. The size must be a compile time constant. If the call to this method is in a loop,
     * always the same pointer is returned. In other words: this method does not allocate memory; it
     * returns the address of a fixed-size block of memory that is reserved in the stack frame when the
     * method starts execution. The memory is not initialized. Two distinct calls of this method return
     * different pointers.
     */
