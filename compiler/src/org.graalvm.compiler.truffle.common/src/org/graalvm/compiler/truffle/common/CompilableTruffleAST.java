...
public interface CompilableTruffleAST {
     ...
     *
     * @param reasonAndStackTrace the output of {@link Throwable#printStackTrace()} for the exception
     *            representing the reason for compilation failure
     * @param bailout specifies whether the failure was a bailout or an error in the compiler. A bailout
     *            means the compiler aborted the compilation based on some of property of the AST (e.g.,
     *            too big). A non-bailout means an unexpected error in the compiler itself.
     * @param permanentBailout specifies if a bailout is due to a condition that probably won't change
     *            if this AST is compiled again. This value is meaningless if {@code bailout == false}.
     */
