...
public abstract class SPARCAssembler extends Assembler {
    ...
    private SpecifiedArrayList<Integer> delaySlotOptimizationPoints = SpecifiedArrayList.createNew(5);
    ...
    public static class SPARCOp {
        ...
        private static final List<SPARCOp> OPS = SpecifiedArrayList.createNew();
        ...
        protected List<BitKey[]> getKeys() {
            if (keyFields == null) {
                keyFields = SpecifiedArrayList.createNew(4);
    ...
    /**
     * Does peephole optimization on code generated by this assembler. This method should be called at
     * the end of code generation.
     * <p>
     * It searches for conditional branch instructions which has nop in the delay slot then looks at the
     * instruction at branch target; if it is an arithmetic instruction, which does not throw an
    ...
    /**
     * Optimizes branch instruction <i>b</t> which has a nop in the delay slot. It tries to stuff the
     * instruction at <i>b</i>s branch target into the delay slot of <i>b</i>, set the annul flag and
     * increments <i>b</i>s disp field by 1;
     * <p>
     * If <i>b</i>s branch target instruction is an unconditional branch <i>t</i>, then it tries to put
     * <i>t</i>s delayed instruction into the delay slot of <i>b</i> and add the <i>t</i>s disp field to
     * <i>b</i>s disp field.
     */
