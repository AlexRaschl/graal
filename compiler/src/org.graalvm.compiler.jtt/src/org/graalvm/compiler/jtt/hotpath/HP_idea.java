...
public class HP_idea extends JTTTest {
     ...
     * calcEncryptKey
     *
     * Builds the 52 16-bit encryption subkeys Z[] from the user key and stores in 32-bit int array. The
     * routing corrects an error in the source code in the Schnier book. Basically, the sense of the 7-
     * and 9-bit shifts are reversed. It still works reversed, but would encrypted code would not
     ...
     * calcDecryptKey
     *
     * Builds the 52 16-bit encryption subkeys DK[] from the encryption- subkeys Z[]. DK[] is a 32-bit
     * int array holding 16-bit values as unsigned.
     */
     ...
     * processing using 6 new subkeys each time, plus four for last step. The source text is in array
     * text1, the destination text goes into array text2 The routine represents 16-bit subblocks and
     * subkeys as type int so that they can be treated more easily as unsigned. Multiplication modulo
     * 0x10001 interprets a zero sub-block as 0x10000; it must to fit in 16 bits.
     */
     ...
     * mul
     *
     * Performs multiplication, modulo (2**16)+1. This code is structured on the assumption that untaken
     * branches are cheaper than taken branches, and that the compiler doesn't schedule branches. Java:
     * Must work with 32-bit int and one 64-bit long to keep 16-bit values and their products
     * "unsigned." The routine assumes that both a and b could fit in 16 bits even though they come in
     * as 32-bit ints. Lots of "& 0xFFFF" masks here to keep things 16-bit. Also, because the routine
     * stores mod (2**16)+1 results in a 2**16 space, the result is truncated to zero whenever the
     * result would zero, be 2**16. And if one of the multiplicands is 0, the result is not zero, but
     * (2**16) + 1 minus the other multiplicand (sort of an additive inverse mod 0x10001).
     *
     * NOTE: The java conversion of this routine works correctly, but is half the speed of using Java's
     * modulus division function (%) on the multiplication with a 16-bit masking of the result--running
     * in the Symantec Caje IDE. So it's not called for now; the test uses Java % instead.
     */
     ...
     *
     * return (b - a + (b < a ? 1 : 0) & 0xFFFF); } else return ((1 - a) & 0xFFFF); // If b = 0, then
     * same as // 0x10001 - a. } else // If a = 0, then return return((1 - b) & 0xFFFF); // same as
     * 0x10001 - b. }
     */
     ...
     * common divisor) algorithm. It is unrolled twice to avoid swapping the meaning of the registers.
     * And some subtracts are changed to adds. Java: Though it uses signed 32-bit ints, the
