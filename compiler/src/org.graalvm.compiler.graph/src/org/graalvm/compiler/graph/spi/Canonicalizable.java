...
public interface Canonicalizable {
    ...
    /**
     * Implementations of this method can provide local optimizations like constant folding and strength
     * reduction. Implementations should look at the properties and inputs of the current node and
     * determine if there is a more optimal and always semantically correct replacement. <br/>
     ...
     * <li>Returning a newly created node (that was not yet added to the graph) will replace the current
     * node with the given one, after adding it to the graph. If both the replacement and the replacee
     * are anchored in control flow (fixed nodes), the replacement will be added to the control flow. It
     * is invalid to replace a non-fixed node with a newly created fixed node (because its placement in
     * the control flow cannot be determined without scheduling).</li>
     * <li>Returning {@code null} will delete the current node and replace it with {@code null} at all
     * usages. Note that it is not necessary to delete floating nodes that have no more usages this way
     * - they will be deleted automatically.</li>
     * </ul>
     *
    ...
    /**
     * This sub-interface of {@link Canonicalizable} is intended for nodes that have exactly one input.
     * It has an additional {@link #canonical(CanonicalizerTool, Node)} method that looks at the given
     * input instead of the current input of the node - which can be used to ask "what if this input is
     * changed to this node" - questions.
     *
    ...
    public interface Unary<T extends Node> extends Canonicalizable {
        ...
        /**
         * Similar to {@link Canonicalizable#canonical(CanonicalizerTool)}, except that implementations
         * should act as if the current input of the node was the given one, i.e., they should never look at
         * the inputs via the this pointer.
         */
        Node canonical(CanonicalizerTool tool, T forValue);

        /**
         * Gets the current value of the input, so that calling {@link #canonical(CanonicalizerTool, Node)}
         * with the value returned from this method should behave exactly like
    ...
    /**
     * This sub-interface of {@link Canonicalizable} is intended for nodes that have exactly two inputs.
     * It has an additional {@link #canonical(CanonicalizerTool, Node, Node)} method that looks at the
     * given inputs instead of the current inputs of the node - which can be used to ask "what if this
    ...
    public interface Binary<T extends Node> extends Canonicalizable {
         ...
         * {@link #canonical(CanonicalizerTool, Node, Node)} with the value returned from this method should
         * behave exactly like {@link Canonicalizable#canonical(CanonicalizerTool)}.
         */
        T getX();
     ...
     * operation is commutative. It is used to improve GVN by trying to merge nodes with the same inputs
     * in different order.
     */
