...
public abstract class ControlSplitNode extends FixedNode implements IterableNodeType {
    ...
    /**
     * Attempts to set the probability for the given successor to the passed value (which has to be in
     * the range of 0.0 and 1.0). Returns whether setting the probability was successful.
     */
    public abstract boolean setProbability(AbstractBeginNode successor, double value);

    /**
     * Primary successor of the control split. Data dependencies on the node have to be scheduled in the
     * primary successor. Returns null if data dependencies are not expected.
     *
